...
import { Activity, Satellite, Radio, Search, X, ChevronLeft, Eye, EyeOff, ZoomIn, ZoomOut } from 'lucide-react'

function UIOverlay({ satelliteData, selectedSat, setSelectedSat, filters, setFilters, cameraControlRef }) {
    ...
    const analytics = useMemo(() => {
        ...
        const leo = satelliteData.filter(s => s.satrec && s.satrec.no > 11.25 * 2 * Math.PI / 1440).length
        ...
        <div className="absolute inset-0 pointer-events-none z-10">
            ...
            >
                <div className="flex items-baseline gap-2">
                    <h1 className="text-lg font-bold tracking-wider text-white" style={{ fontFamily: 'Rajdhani, monospace' }}>
                        NEBULA LEDGER
                    </h1>
                    <span className="text-cyan-400 text-sm">//</span>
                    <span className="text-sm text-white/70 tracking-widest font-mono">SATELLITE TRACKING</span>
                </div>
            ...
            >
                <div className="bg-black/80 backdrop-blur-md border border-white/10 rounded-2xl p-6 h-full shadow-2xl flex flex-col">
                    ...
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        <h2 className="text-xl font-bold text-white tracking-wide">SATELLITE CATALOG</h2>
                    </div>
                    ...
                    <div className="text-xs text-white/50 mt-4 mb-2">
                        Showing {Math.min(filteredSatellites.length, 20)} of {filteredSatellites.length} results
                    </div>
                    ...
                    <div className="flex-grow overflow-y-auto space-y-2 pr-2">
                        {filteredSatellites.slice(0, 20).map((sat, index) => {
                            ...
                            const accentColor = sat.type === "Debris" ? "text-red-400" :
                                sat.type === "GPS" ? "text-yellow-400" :
                                    sat.type === "Starlink" ? "text-white" : "text-cyan-400"
                            const bgAccent = isSelected ? "bg-cyan-500/20" :
                                sat.type === "Debris" ? "bg-red-500/10" :
                                    "bg-cyan-500/5"
                            const borderAccent = isSelected ? "border-cyan-500/50" :
                                sat.type === "Debris" ? "border-red-500/30" : "border-white/10"
                            ...
                            return (
                                ...
                                >
                                    <div className="flex items-start justify-between">
                                        ...
                                        <div className={`w-2 h-2 rounded-full mt-1 flex-shrink-0`} style={{ backgroundColor: sat.color }}></div>
                                    </div>
                                </motion.div>
                            )
                        })}
                    ...
                    <div className="mt-auto pt-4 border-t border-white/10">
                        <div className="bg-black/90 rounded-xl p-4">
                            <h3 className="text-sm font-bold text-white/90 mb-3 tracking-wide" style={{ fontFamily: 'Rajdhani, sans-serif' }}>
                                OBJECT CLASSIFICATION
                            </h3>
                            <div className="space-y-2">
                                <div className="flex items-center gap-3">
                                    <div className="w-2 h-2 rounded-full bg-red-500 flex-shrink-0"></div>
                                    <span className="text-xs text-white/70">RED: Debris / Uncontrolled</span>
                                </div>
                                <div className="flex items-center gap-3">
                                    <div className="w-2 h-2 rounded-full bg-cyan-400 flex-shrink-0"></div>
                                    <span className="text-xs text-white/70">CYAN: Station / Manned</span>
                                </div>
                                <div className="flex items-center gap-3">
                                    <div className="w-2 h-2 rounded-full bg-white flex-shrink-0"></div>
                                    <span className="text-xs text-white/70">WHITE: Active Satellite</span>
                                </div>
                                <div className="flex items-center gap-3">
                                    <div className="w-2 h-2 rounded-full bg-yellow-400 flex-shrink-0"></div>
                                    <span className="text-xs text-white/70">GOLD: GPS / Navigation</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="bg-black/80 backdrop-blur-md border border-white/10 rounded-2xl p-6 flex-1 shadow-2xl overflow-y-auto flex flex-col gap-6 scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent">
                    ...
                    <div>
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <h2 className="text-xl font-bold text-white tracking-wide">ORBITAL ANALYTICS</h2>
                        </div>
                        ...
                        <div className="space-y-3">
                            ...
                            >
                                <div className="text-sm text-cyan-400 font-medium mb-1">TOTAL TRACKED</div>
                            ...
                            >
                                <div className="text-sm text-red-400 font-medium mb-1">DEBRIS COUNT</div>
                                <div className="text-4xl font-bold text-white tabular-nums">
                                    {analytics.debris.toLocaleString()}
                                </div>
                                <div className="text-xs text-white/50 mt-1">Collision Hazards</div>
                            </motion.div>
                            ...
                            >
                                <div className="text-sm text-purple-400 font-medium mb-1">STATIONS</div>
                                <div className="text-4xl font-bold text-white tabular-nums">
                                    {analytics.stations.toLocaleString()}
                                </div>
                                <div className="text-xs text-white/50 mt-1">Operational Facilities</div>
                            </motion.div>
                            ...
                            >
                                <div className="text-sm text-yellow-400 font-medium mb-1">LEO DENSITY</div>
                                <div className="text-4xl font-bold text-white tabular-nums">
                                    {analytics.leoDensity}%
                                </div>
                                <div className="text-xs text-white/50 mt-1">Low Earth Orbit</div>
                            </motion.div>
                        </div>
                    </div>
                    ...
                    >
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <h2 className="text-xl font-bold text-white tracking-wide">VISIBILITY FILTERS</h2>
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <button
                                ...
                                className={`w-full flex items-center justify-between p-3 rounded-lg border transition-all ${filters.debris
                            ...
                            >
                                ...
                                {filters.debris ? <Eye className="w-4 h-4 text-red-400" /> : <EyeOff className="w-4 h-4 text-white/40" />}
                            </button>

                            {/* Station Filter */}
                            <button
                                onClick={() => toggleFilter('station')}
                                className={`w-full flex items-center justify-between p-3 rounded-lg border transition-all ${filters.station
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-sm text-white font-medium">Stations</span>
                                </div>
                                {filters.station ? <Eye className="w-4 h-4 text-cyan-400" /> : <EyeOff className="w-4 h-4 text-white/40" />}
                            </button>

                            {/* Starlink Filter */}
                            <button
                                onClick={() => toggleFilter('starlink')}
                                className={`w-full flex items-center justify-between p-3 rounded-lg border transition-all ${filters.starlink
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-sm text-white font-medium">Starlink</span>
                                </div>
                                {filters.starlink ? <Eye className="w-4 h-4 text-white" /> : <EyeOff className="w-4 h-4 text-white/40" />}
                            </button>

                            {/* GPS Filter */}
                            <button
                                onClick={() => toggleFilter('gps')}
                                className={`w-full flex items-center justify-between p-3 rounded-lg border transition-all ${filters.gps
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-sm text-white font-medium">GPS / Navigation</span>
                                </div>
                                {filters.gps ? <Eye className="w-4 h-4 text-yellow-400" /> : <EyeOff className="w-4 h-4 text-white/40" />}
                            </button>
                        </div>
                    </motion.div>
                </div>
                ...
                >
                    <div className="bg-black/80 backdrop-blur-md border border-white/20 rounded-2xl px-8 py-5 shadow-2xl min-w-[500px]">
                        <div className="flex items-start justify-between mb-4">
                            <div>
                                <h3 className="text-xl font-bold text-white mb-1">{selectedSat.name}</h3>
                        ...
                        <div className="grid grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <div className="text-white font-mono text-sm">
                                    {selectedSat.satrec?.inclo ? (selectedSat.satrec.inclo * 180 / Math.PI).toFixed(2) + '°' : 'N/A'}
                                </div>
                            </div>
                            <div>
                                <div className="text-xs text-white/50 mb-1">PERIOD</div>
                                <div className="text-white font-mono text-sm">
                                    {selectedSat.satrec?.no ? ((2 * Math.PI) / selectedSat.satrec.no).toFixed(1) + ' min' : 'N/A'}
                                </div>
                            </div>
                        </div>
